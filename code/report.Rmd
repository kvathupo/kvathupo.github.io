...
abstract: "In this project, we compare the performance of two portfolios, \\
  ...
  the other by humans. The former portfolio is constructed \\
...
succeed over the monkeys, the answer is affirmative. The "monkey" 
portfolio is merely a proxy for a randomly constructed one. The specific
...
physical [models of Brownian motion](https://en.wikipedia.org/wiki/Ornstein%E2%80%93Uhlenbeck_process#Application_in_financial_mathematics) have found use in finance.
...
sequentially:
  ...
  1. The construction of the monkey portfolio
     ...
     + Each trading week, select a stock. Buy as many shares of this stock as 
     possible. At week's end, sell all shares and repeat this process for 
     another, randomly chosen stock.
     + Test Period: Jan. 2017 to May 2019
     
  2. The construction of the human portfolio
     + Initial capital of 1,000,000 dollars
     + Each trading week, select the stock with the highest expected return, 
     as determined by our regression model. Buy as many shares of this stock as
     possible. At week's end, sell all shares and repeat this process for 
     another stock.
...
<p class="note" align="center">
  **Note**: A trading week is not 7 days (traders need weekends off too)! 
  A trading week is typically 5 days. We extrapolate the number of trading
  days in a month and year similarly from average metrics.
</p>

As stated previously, the "monkey" portfolio is merely a proxy term for 
a randomly-constructed portfolio devoid of financial theory. The human
portfolio applies Arbitrage Pricing Theory to model daily stock returns as 
...
here: https://iextrading.com/developer/docs/ 
Let's get stock data over the last 2 years for Apple to start.
...
Congrats! We have some data. However, as you may notice, there isn't any way 
to conveniently use this data.  We want to get the data into a more usable 
format.  Let's start by unpacking the JSON formatted data into a more readable
format:
...
Now monkeys aren't very smart, so they will only care about the closing price 
...
over a specific period, so let's also filter out any rows that don't fall 
...
<p class="note" align="center">
  **Note**: This data is dynamically generated from our JSON call. Thus, it is
  execution dependent. Recall that we recorded the performance of our monkey
  portfolio in a single run by CSV file in order to allow stable comparison
  with our human portfolio.
</p>
...
<p class="note" align="center">
  **Note**: In practice, this was done once so as to minimize the tutorial's
  dependence on the time of JSON call.
</p>
...
For the sake of brevity, we provide a high-level overview. If this is still not sufficient, consult the textbook
above.
...
risk-free asset with one of little risk, such as 10 year U.S. Treasury notes
(T-Bill).
...
time-intervals, namely our weekly holding period. 
...
assets that can be lowered by adjusting our portfolio's exposure to said asset.
...
compensated for risk that can be diversified, we would have arbitrage.
...
$(r_{f})_i$ is the return of the risk-less asset in the $i$th month.
...
Now, we prepare our raw data for the calculation of our factors. The contents
of the dataframes are shown.
...
### Consolidated dataframes {.tabset .tabset-fade}
The contents of each dataframe, after combining business sheet and daily
data.
...
### Regression-ready dataframes {.tabset .tabset-fade}
...
# Comparing our Monkey and Human Portfolios

It's clear from the graph that the human portfolio outperforms the monkey
portfolio. Having both stated with 1,000,000 dollars, both portfolios
earned a profit, but the human portfolio earned the most.
...
## Final Portfolio Values {.tabset .tabset-fade}

### Human Portfolio

Our human portfolio returned the following amount after the training 
period:
```{r, human_net}
...
Our monkey portfolio returned the following amount after the training 
period:
```{r, monkey_net}
...
Our tutorial confirms our initial hypothesis that humans outperform the 
theoretical monkeys in the arena of asset management. It does not, however,
indicate that our particular strategy is ready for deployment in the real 
world. In application, our strategy's returns would be diminished with
the consideration of trading costs, be they liquidity demands or brokerage
fees. Furthermore, we simplified many complications, which, in the real 
world, are unavoidable. For example, we can't own fractions of a share, 
like in our program!

Furthermore, the theory of Arbitrage Pricing Theory neglects salient 
statistical metrics that are commonly used in data science to assess the 
applicability of a model, namely $p$-value assessment. Considering the 
overall design of our experiment, we could have returned more significant results
by considering multiple monkey portfolios. As it stands, it is not sufficient
to merely use one instance of a monkey portfolio. This decision, however, was
made out of time constraints.

In conclusion, our tutorial confirms our hypothesis. That being said, our model
is far from industry-ready. It can, however, be improved by combining concepts 
in data science and financial theory.


# References

  * For more information on the theoretical foundations of APT, consult
     + Financial Modeling of the Equity Market: From CAPM to Cointegration, 
     available [here](https://onlinelibrary.wiley.com/doi/book/10.1002/9781119201236)
  * For more information on factor choice, consult
     + Barra's Risk Model Handbook, available [here](https://www.msci.com/www/research-paper/barra-s-risk-models/014972229)
...
10-Q SEC filings (10-K and 10-Q forms respectively are annual and quarterly 
